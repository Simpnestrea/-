...
@section('title', 'Kho Món Ngon - Foodball')
@section('header_title', 'Kho Món Ngon')
    ...
    <div class="flex-1 max-w-5xl mx-auto w-full px-6 py-10 space-y-10 relative">
        ...
        <div class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div class="flex items-center space-x-6">
                @if(auth()->user()->avatar)
                    <img src="{{ str_contains(auth()->user()->avatar, 'http') ? auth()->user()->avatar : Storage::url(auth()->user()->avatar) }}" class="w-24 h-24 rounded-full border-4 border-white shadow-md object-cover">
                @else
                ...
                <div>
                    <div class="flex items-center space-x-3">
                        <h1 class="text-2xl font-black mb-1">Bếp của {{ auth()->user()->name }}</h1>
                        <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border
                            @if(auth()->user()->role === 'beginner') bg-slate-100 text-slate-700 border-slate-200
                            @elseif(auth()->user()->role === 'homecook') bg-blue-50 text-blue-700 border-blue-100
                            @elseif(auth()->user()->role === 'prochef') bg-emerald-50 text-emerald-700 border-emerald-100
                            @elseif(auth()->user()->role === 'masterchef') bg-orange-50 text-orange-700 border-orange-100
                    ...
                    <p class="text-gray-500 font-medium mt-1">
                        Thành viên từ {{ auth()->user()->created_at ? 'Tháng ' . auth()->user()->created_at->format('m, Y') : 'Tháng 5, 2026' }}
                    </p>
                </div>
            </div>
            <a href="{{ route('recipe.create') }}" class="bg-orange-500 text-white px-6 py-3 rounded-xl font-bold hover:bg-orange-600 transition shadow-md flex items-center space-x-2">
        ...
        <div class="grid grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center">
                <div id="mine-count" class="text-3xl font-black text-gray-900 mb-1">{{ $myRecipesCount }}</div>
                <div class="text-sm font-semibold text-gray-500 uppercase">Công Thức Đã Viết</div>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center">
                <div id="saved-count" class="text-3xl font-black text-gray-900 mb-1">{{ $savedRecipesCount }}</div>
                <div class="text-sm font-semibold text-gray-500 uppercase">Đã Lưu</div>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center">
                <div class="text-3xl font-black text-gray-900 mb-1">{{ $likesReceivedCount }}</div>
                <div class="text-sm font-semibold text-gray-500 uppercase">Lượt Thích Nhận Được</div>
            </div>
        </div>

        <!-- Rank Progress Card -->
        @if(auth()->user()->role === 'masterchef')
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                        <h3 class="text-sm font-extrabold text-orange-850 flex items-center gap-1.5">
                            <span>🚀</span> Lộ trình thăng cấp tiếp theo: <span class="underline font-black">{{ $targetRoleName }}</span>
                        </h3>
                    ...
                    <div class="text-right">
                        <span class="text-xs font-black text-orange-700 font-mono bg-orange-100/50 px-2.5 py-1 rounded-lg">{{ round($overallProgress) }}% Hoàn thành</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex justify-between text-xs font-bold text-gray-700">
                            <span>Đăng công thức ({{ $myRecipesCount }} / {{ $targetRecipes }})</span>
                            <span class="font-mono">{{ round($recipeProgress) }}%</span>
                        </div>
                        <div class="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                            <div class="bg-orange-500 h-full rounded-full transition-all duration-500" style="width: {{ $recipeProgress }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        <div class="flex justify-between text-xs font-bold text-gray-700">
                            <span>Lượt thích nhận được ({{ $likesReceivedCount }} / {{ $targetLikes }})</span>
                            <span class="font-mono">{{ round($likeProgress) }}%</span>
                        </div>
                        <div class="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                            <div class="bg-amber-500 h-full rounded-full transition-all duration-500" style="width: {{ $likeProgress }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div x-data="{ 
            ...
            hasVisibleSaved() {
                ...
                const titles = [
                    ...
                    this.removeAccents('{{ addslashes($r->title) }}'),
                    @endforeach
                ];
            ...
            <div x-show="tab === 'saved'" class="space-y-6">
                <div x-show="hasVisibleSaved()" class="grid md:grid-cols-3 gap-6">
                    @forelse($savedRecipes as $savedRecipe)
                        <div x-show="!searchQuery || removeAccents('{{ addslashes($savedRecipe->title) }}').includes(removeAccents(searchQuery))" 
                            ...
                            <a href="{{ route('recipe.detail', $savedRecipe->slug) }}" class="block">
                                <div class="w-full h-40 overflow-hidden bg-gray-50 flex items-center justify-center relative">
                                    @if($savedRecipe->image)
                                        <img src="{{ $savedRecipe->image }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                                    @else
                                    ...
                                    <!-- Unsave Button -->
                                    <button @click.stop.prevent="unsaveRecipe({{ $savedRecipe->id }}, $event)" 
                                ...
                                <div class="p-4">
                                    <h3 class="font-bold text-lg mb-1 line-clamp-1 text-gray-900 group-hover:text-orange-500 transition">{{ $savedRecipe->title }}</h3>
                                    <p class="text-gray-500 text-sm">Bởi {{ $savedRecipe->user->name }}</p>
                                </div>
                            </a>
                        </div>
                    @empty
            ...
            <div x-show="tab === 'mine'" style="display: none;" class="space-y-6">
                <div x-show="hasVisibleMine()" class="grid md:grid-cols-3 gap-6">
                    @forelse($myRecipes as $myRecipe)
                        <div x-show="!searchQuery || removeAccents('{{ addslashes($myRecipe->title) }}').includes(removeAccents(searchQuery))" 
                            ...
                            <a href="{{ route('recipe.detail', $myRecipe->slug) }}" class="block">
                                <div class="w-full h-40 overflow-hidden bg-gray-50 flex items-center justify-center relative">
                                    @if($myRecipe->image)
                                        <img src="{{ $myRecipe->image }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                                    @else
                                        <svg class="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    @endif
                                ...
                                <div class="p-4 pb-2">
                                    <h3 class="font-bold text-lg mb-1 line-clamp-1 text-gray-900 group-hover:text-orange-500 transition">{{ $myRecipe->title }}</h3>
                            ...
                            <div class="px-4 pb-4 pt-2 flex items-center gap-2 border-t border-gray-50 bg-gray-50/50">
                                <a href="{{ route('recipe.edit', $myRecipe) }}" 
                                ...
                                <form action="{{ route('recipe.destroy', $myRecipe) }}" method="POST" class="m-0 flex-1" @submit.prevent="deleteRecipe({{ $myRecipe->id }}, $event)">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <div class="col-span-full bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center py-20 text-center w-full">
                            ...
                            <a href="{{ route('recipe.create') }}" class="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2.5 px-6 rounded-full transition shadow-md shadow-orange-500/20">Viết Công Thức</a>
                        </div>
                    @endforelse
                </div>
