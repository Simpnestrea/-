...
<head>
    ...
    <title>Viết món mới - Foodball</title>
...
<body class="bg-gray-50 text-gray-800 h-screen flex flex-col overflow-hidden">
    ...
    <form action="{{ route('recipe.store') }}" method="POST" enctype="multipart/form-data" class="h-full flex flex-col" x-data="{ ingredients: [{ id: 1 }], steps: [{ id: 1 }] }">
        @csrf
        
        <header class="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between sticky top-0 z-20 shrink-0">
            <a href="{{ route('home') }}" class="text-gray-500 hover:text-orange-500 font-bold text-sm">Hủy</a>
            <div class="font-black text-xl tracking-tight text-gray-900 flex items-center space-x-2">
                <span>Viết công thức mới</span>
                <span>🍳</span>
            </div>
            <button type="submit" class="bg-orange-500 text-white px-6 py-2 rounded-lg font-bold text-sm hover:bg-orange-600 transition">Đăng</button>
        </header>
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-8">
            ...
            <div class="max-w-6xl mx-auto bg-white rounded-xl border border-gray-200 p-6 sm:p-10 flex flex-col lg:flex-row gap-10">
                ...
                <div class="w-full lg:w-[360px] shrink-0 space-y-8">
                    ...
                    <label class="block w-full aspect-square bg-gray-50 border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 hover:text-orange-500 hover:border-orange-400 transition cursor-pointer relative overflow-hidden rounded-xl" x-data="{ preview: null }">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center space-x-3 mb-4">
                            <span class="text-sm font-medium text-gray-600">Khẩu phần:</span>
                            <input type="text" placeholder="2 người" class="border border-gray-300 rounded px-3 py-1 text-sm w-28 outline-none focus:border-orange-500">
                        </div>
                        ...
                        <div class="space-y-3">
                            <template x-for="(ing, index) in ingredients" :key="ing.id">
                                <div class="flex items-center space-x-2 group">
                                    ...
                                    <input type="text" :name="`ingredients[${index}][quantity]`" placeholder="Lượng (vd: 200)" class="w-20 border border-gray-300 rounded px-2 py-1.5 text-sm outline-none focus:border-orange-500">
                                    <input type="text" :name="`ingredients[${index}][unit]`" placeholder="Đơn vị (vd: g)" class="w-16 border border-gray-300 rounded px-2 py-1.5 text-sm outline-none focus:border-orange-500">
                                    <input type="text" :name="`ingredients[${index}][name]`" placeholder="Tên (vd: Thịt bò)" class="flex-1 min-w-0 border border-gray-300 rounded px-2 py-1.5 text-sm outline-none focus:border-orange-500" required>
                                    <button type="button" @click="ingredients.splice(index, 1)" class="text-red-400 hover:text-red-600 font-bold px-1 text-lg">×</button>
                                </div>
                            </template>
                        </div>

                        <div class="flex pt-2">
                            <button type="button" @click="ingredients.push({ id: Date.now() })" class="text-sm font-bold text-orange-500 hover:text-orange-600">+ Thêm nguyên liệu</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-1 space-y-8 lg:border-l lg:border-gray-200 lg:pl-10">
                    ...
                    <div class="flex items-center space-x-4 border border-gray-300 px-4 py-2 rounded-xl">
                        ...
                        <input type="text" name="title" id="title" class="flex-1 border-none bg-transparent outline-none py-1 text-gray-900 font-medium placeholder-gray-400" placeholder="Ví dụ: Bún chả Hà Nội..." required>
                    </div>
                    ...
                    <div class="bg-amber-50/50 border border-amber-200/60 p-5 rounded-2xl space-y-4" x-data="{ isPremium: false }">
                        ...
                        <div x-show="isPremium" x-transition class="space-y-2" style="display: none;">
                            <div class="flex items-center space-x-3 border border-gray-300 px-4 py-2.5 rounded-xl bg-white">
                                ...
                                <input type="number" name="price" id="price" class="flex-1 border-none bg-transparent outline-none py-1 text-gray-900 font-bold placeholder-gray-400 text-sm" placeholder="Ví dụ: 50000" min="1000" step="1000">
                    ...
                    <!-- Mô tả công thức -->
                    <textarea name="description" class="w-full h-32 border border-gray-300 p-4 text-gray-700 outline-none focus:border-orange-500 resize-none rounded-xl" placeholder="Mô tả ngắn gọn về món ăn của bạn (câu chuyện, cảm xúc, hương vị...)..."></textarea>
                    ...
                    <div class="space-y-6 border-t border-gray-200 pt-8">
                        ...
                        <div class="flex items-center space-x-3 mb-6">
                            ...
                            <input type="number" name="time_to_cook" id="time_to_cook" placeholder="30" min="1" class="border border-gray-300 rounded px-3 py-1 text-sm w-24 outline-none focus:border-orange-500" required>
                        ...
                        <div class="space-y-8">
                            <template x-for="(step, index) in steps" :key="step.id">
                                <div class="flex items-start space-x-3 bg-gray-50/50 p-4 rounded-xl border border-gray-100" x-data="{ stepPreview: null }">
                                    ...
                                    <div class="flex-1 space-y-3">
                                        <div class="flex items-start space-x-2">
                                            <textarea :name="`steps[${index}][content]`" placeholder="Mô tả cách thực hiện bước này..." class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-orange-500 h-20 resize-none" required></textarea>
                                            <button type="button" @click="steps.splice(index, 1)" class="text-red-400 hover:text-red-600 font-bold px-1 text-lg">×</button>
                                        </div>
                        ...
                        <div class="pt-4 text-center sm:text-left sm:pl-16">
                            <button type="button" @click="steps.push({ id: Date.now() })" class="text-sm font-bold text-orange-500 hover:text-orange-600">+ Thêm bước làm</button>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-8">
                        ...
                        <textarea name="tips" id="tips" class="w-full h-20 border border-gray-300 p-3 text-gray-700 outline-none focus:border-orange-500 resize-none rounded-xl" placeholder="Chia sẻ các mẹo nhỏ, lưu ý khi nấu món này..."></textarea>
                    </div>
