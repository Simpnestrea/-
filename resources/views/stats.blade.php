...
@section('title', 'Thống Kê Bếp - Foodball')
@section('header_title', 'Thống Kê Bếp')
    ...
    <div class="flex-1 max-w-5xl mx-auto w-full px-6 py-12 space-y-8 relative">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center space-x-4 hover:shadow-md transition">
                ...
                <div>
                    <div class="text-3xl font-black">{{ number_format($likesCount) }}</div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center space-x-4 hover:shadow-md transition">
                ...
                <div>
                    <div class="text-3xl font-black">{{ number_format($savesCount) }}</div>
                ...
                @forelse($topRecipes as $recipe)
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-orange-50/50 transition">
                        <div class="flex items-center space-x-4">
                            @if($recipe->image)
                                <img src="{{ str_contains($recipe->image, 'http') ? $recipe->image : Storage::url($recipe->image) }}" class="w-12 h-12 rounded-lg object-cover shadow-sm">
                            @else
                                <div class="w-12 h-12 rounded-lg bg-orange-100 text-orange-600 flex items-center justify-center font-bold text-lg shadow-sm shrink-0">
                                    {{ substr($recipe->title, 0, 1) }}
                                </div>
                            @endif
                            <div>
                                <a href="{{ route('recipe.detail', $recipe->slug) }}" class="font-bold text-gray-900 hover:text-orange-500 transition block">
                                    {{ $recipe->title }}
                                </a>
                                <p class="text-sm text-gray-500">Đăng ngày {{ $recipe->created_at->format('d/m/Y') }} • {{ number_format($recipe->views_count) }} lượt xem</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <div class="font-bold text-orange-600 flex items-center justify-end gap-1.5 text-sm">
                                <span>+{{ $recipe->likes_in_period }} ❤️</span>
                                <span>+{{ $recipe->saves_in_period }} 🔖</span>
                            </div>
