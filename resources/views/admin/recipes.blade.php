    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <form action="{{ route('admin.recipes') }}" method="GET" class="flex-1 flex flex-col sm:flex-row gap-3">
            ...
            <select name="difficulty" onchange="this.form.submit()" class="py-2.5 pl-4 pr-10 rounded-xl border border-slate-200 outline-none text-sm transition-all focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10">
                ...
                <option value="dễ" {{ ($difficulty ?? '') === 'dễ' ? 'selected' : '' }}>Dễ</option>
                <option value="trung bình" {{ ($difficulty ?? '') === 'trung bình' ? 'selected' : '' }}>Trung bình</option>
                <option value="khó" {{ ($difficulty ?? '') === 'khó' ? 'selected' : '' }}>Khó</option>
            </select>
            ...
            <select name="premium_status" onchange="this.form.submit()" class="py-2.5 pl-4 pr-10 rounded-xl border border-slate-200 outline-none text-sm transition-all focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10">
                ...
                <option value="premium" {{ ($premiumStatus ?? '') === 'premium' ? 'selected' : '' }}>Bài Premium</option>
                <option value="free" {{ ($premiumStatus ?? '') === 'free' ? 'selected' : '' }}>Bài Thường</option>
            </select>
        </form>
                        ...
                        <tr class="hover:bg-slate-50/30 transition-all" id="recipe-row-{{ $recipe->id }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <button @click="showDetail({{ $recipe->id }})" class="bg-slate-50 hover:bg-slate-100 text-slate-750 font-bold px-3 py-1.5 rounded-lg border border-slate-200 text-xs transition">
                                        Xem chi tiết
                                    </button>
