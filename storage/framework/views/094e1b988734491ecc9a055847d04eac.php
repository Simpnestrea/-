        ...
        <div class="px-10 pb-10 pt-6 max-w-5xl mx-auto w-full space-y-8 flex-1">
            ...
            <div class="text-center space-y-3" x-data="{
                ...
                <div class="relative w-full max-w-2xl mx-auto z-50">
                    <form action="<?php echo e(route('search')); ?>" method="GET" class="relative flex w-full items-center group">
                ...
                <div class="flex items-center justify-center gap-2 flex-wrap text-sm text-gray-400 pt-1">
                    <span>Gợi ý:</span>
                    <?php $__currentLoopData = ['Phở Bò', 'Bánh Mì', 'Gỏi Cuốn', 'Cơm Tấm', 'Lẩu']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $suggest): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e(route('search', ['query' => $suggest])); ?>" class="bg-gray-100 hover:bg-orange-100 hover:text-orange-600 text-gray-500 px-3 py-1 rounded-full transition font-medium"><?php echo e($suggest); ?></a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>

            <div x-data="{ 
                activeSlide: <?php echo e($featuredRecipes->first()->id ?? 1); ?>, 
                slides: <?php echo e(\Illuminate\Support\Js::from($featuredRecipes->map(fn($r) => [
                    'id' => $r->id, 
                    'title' => $r->title, 
            ...
            <section class="space-y-8">
                <div class="flex justify-between items-end border-b-2 border-gray-100 pb-3">
                    ...
                    <span class="text-sm text-gray-400">Cập nhật lúc: <?php echo e(now()->format('H:i')); ?></span>
                </div>
                
                <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e(route('search', ['type' => 'category', 'category' => $category->slug])); ?>" class="group relative rounded-xl overflow-hidden aspect-video border border-gray-100 hover:shadow-xl transition-all shadow-sm">
                            <img src="<?php echo e($category->image); ?>" class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                            <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                            <div class="absolute bottom-4 left-4 z-10 text-white font-bold"><?php echo e($category->name); ?></div>
                        </a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </section>

            <?php if(isset($premiumRecipes) && $premiumRecipes->isNotEmpty()): ?>
            <section class="space-y-5">
                <div class="flex items-center justify-between border-b-2 border-gray-100 pb-3">
                    ...
                    <a href="<?php echo e(route('premium')); ?>" class="text-sm font-bold text-orange-500 hover:underline whitespace-nowrap">Đăng ký Premium →</a>
                </div>
                ...
                <div class="relative group/carousel">
                    ...
                    <div id="carousel-premium" class="flex gap-5 overflow-x-auto pb-3 scroll-smooth" style="scrollbar-width: none; -ms-overflow-style: none;">
                        <?php $__currentLoopData = $premiumRecipes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $recipe): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <a href="<?php echo e(route('recipe.detail', $recipe->slug)); ?>"
                               class="group/card flex-none w-56 bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl transition-all flex flex-col relative">
                                <div class="h-36 relative overflow-hidden shrink-0">
                                    <img src="<?php echo e($recipe->image); ?>" class="absolute inset-0 w-full h-full object-cover group-hover/card:scale-110 transition-transform duration-500" alt="<?php echo e($recipe->title); ?>">
                                    <div class="absolute top-2 left-2 bg-amber-500 text-white text-[10px] font-black px-2 py-0.5 rounded-lg shadow-sm">👑 <?php echo e($recipe->category->name ?? 'Premium'); ?></div>
                                    <div class="absolute bottom-2 right-2 bg-black/80 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-0.5 rounded-lg shadow-sm font-mono"><?php echo e(number_format($recipe->price, 0, ',', '.')); ?> đ</div>
                                </div>
                                <div class="p-4 flex flex-col flex-1">
                                    <h3 class="font-bold text-sm text-gray-900 mb-1 group-hover/card:text-orange-500 transition line-clamp-2 leading-snug"><?php echo e($recipe->title); ?></h3>
                                    <p class="text-xs text-gray-400 line-clamp-2 flex-1 mb-3"><?php echo e(Str::limit($recipe->description, 60)); ?></p>
            ...
            <section class="space-y-5">
                <div class="flex items-center justify-between border-b-2 border-gray-100 pb-3">
                    <h2 class="text-2xl font-black uppercase tracking-tight text-gray-900"><?php echo e($section['title']); ?></h2>
                    <a href="<?php echo e(route('search', ['query' => $section['link_query']])); ?>" class="text-sm font-bold text-orange-500 hover:underline whitespace-nowrap">Xem tất cả →</a>
                </div>

                <div class="relative group/carousel">
                    
                    <button onclick="document.getElementById('<?php echo e($section['id']); ?>').scrollBy({left: -340, behavior: 'smooth'})"
                        class="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 z-10 w-10 h-10 bg-white border border-gray-200 rounded-full shadow-lg flex items-center justify-center opacity-0 group-hover/carousel:opacity-100 transition hover:bg-orange-50 hover:border-orange-300">
                        <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                    </button>

                    
                    <div id="<?php echo e($section['id']); ?>" class="flex gap-5 overflow-x-auto pb-3 scroll-smooth" style="scrollbar-width: none; -ms-overflow-style: none;">
                        <?php $__empty_1 = true; $__currentLoopData = $section['recipes']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $recipe): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <a href="<?php echo e(route('recipe.detail', $recipe->slug)); ?>"
                               class="group/card flex-none w-56 bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl transition-all flex flex-col">
                                <div class="h-36 relative overflow-hidden shrink-0">
                                    <img src="<?php echo e($recipe->image); ?>" class="absolute inset-0 w-full h-full object-cover group-hover/card:scale-110 transition-transform duration-500" alt="<?php echo e($recipe->title); ?>">
                                    <div class="absolute top-2 left-2 bg-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-lg shadow-sm"><?php echo e($recipe->category->name ?? 'Nổi Bật'); ?></div>
                                </div>
                                <div class="p-4 flex flex-col flex-1">
                                    <h3 class="font-bold text-sm text-gray-900 mb-1 group-hover/card:text-orange-500 transition line-clamp-2 leading-snug"><?php echo e($recipe->title); ?></h3>
                                    <p class="text-xs text-gray-400 line-clamp-2 flex-1 mb-3"><?php echo e(Str::limit($recipe->description, 60)); ?></p>
                                    <div class="flex items-center justify-between text-[11px] text-gray-400 font-medium border-t border-gray-50 pt-2">
                                        ...
                                        <span class="px-2 py-0.5 rounded-full text-[10px] font-bold
                                            <?php echo e($recipe->difficulty === 'dễ' ? 'bg-green-100 text-green-700' : ($recipe->difficulty === 'khó' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700')); ?>">
                                            <?php echo e(ucfirst($recipe->difficulty)); ?>
            ...
            <div class="bg-black text-white rounded-[30px] p-10 flex flex-col md:flex-row items-center justify-between border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                ...
                <a href="<?php echo e(route('premium')); ?>" class="mt-6 md:mt-0 bg-white text-black px-10 py-4 rounded-full font-black hover:bg-orange-500 hover:text-white transition uppercase text-sm border-2 border-black">
                    Bắt Đầu ngay
                </a>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\--main\resources\views/welcome.blade.php ENDPATH**/ ?>