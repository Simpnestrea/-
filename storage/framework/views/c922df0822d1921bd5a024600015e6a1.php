<?php $__env->startSection('title', $recipe->title . ' - Foodball'); ?>
<?php $__env->startSection('header_title', Str::limit($recipe->title, 30)); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    ?>
    <div x-data="{
        recipeLiked: <?php echo e($isLiked ? 'true' : 'false'); ?>,
        recipeLikesCount: <?php echo e($likesCount); ?>,
        recipeSaved: <?php echo e($isSaved ? 'true' : 'false'); ?>,
        async toggleRecipeLike() {
            try {
                const response = await fetch('<?php echo e(route('recipe.like', $recipe->id)); ?>', {
                    method: 'POST',
                    headers: {
        ...
        async toggleRecipeSave() {
            try {
                const response = await fetch('<?php echo e(route('recipe.save', $recipe->id)); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
        ...
        async unlockRecipe() {
            try {
                const response = await fetch('<?php echo e(route('recipe.unlock-via-ad', $recipe->id)); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
        ...
        <?php if(session('error')): ?>
            <div class="p-4 text-sm text-red-800 rounded-2xl bg-red-50 border border-red-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 shadow-[0_4px_12px_rgba(239,68,68,0.05)]" role="alert">
                ...
                <?php if(str_contains(session('error'), 'nạp thêm tiền') || str_contains(session('error'), 'Số dư') || str_contains(session('error'), 'không đủ')): ?>
                    <a href="<?php echo e(route('wallet.index')); ?>" class="shrink-0 bg-red-600 hover:bg-red-750 text-white text-xs font-bold px-4 py-2 rounded-xl transition shadow-md shadow-red-500/20">
                        Nạp tiền ngay 💳
                    </a>
                <?php endif; ?>
            </div>
        ...
        <div class="border border-gray-300 p-6 rounded-lg flex flex-col lg:flex-row gap-8">
            ...
            <div class="w-full lg:w-[350px] aspect-square bg-gray-50 border border-gray-300 flex items-center justify-center shrink-0 rounded overflow-hidden">
                <?php if($recipe->image): ?>
                    <img src="<?php echo e($recipe->image); ?>" alt="<?php echo e($recipe->title); ?>" class="w-full h-full object-cover">
                <?php else: ?>
            ...
            <div class="flex-1 space-y-5">
                <h1 class="text-3xl font-black"><?php echo e($recipe->title); ?></h1>
                <div class="flex items-center space-x-2 text-orange-500 font-bold">
                    <span>#<?php echo e($recipe->category->name ?? 'Món ăn'); ?></span>
                </div>
                ...
                <div class="flex space-x-3 pt-2">
                    ...
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="border border-gray-300 text-gray-600 hover:bg-gray-50 px-4 py-1.5 rounded flex items-center justify-center transition font-bold space-x-1.5 text-sm">
                            <span>🤍</span>
                            <span>Thích (<?php echo e($likesCount); ?>)</span>
                        </a>
                    <?php endif; ?>
                    ...
                    <button onclick="document.getElementById('comments-section').scrollIntoView({ behavior: 'smooth' })" class="border border-gray-300 px-4 py-1.5 rounded flex items-center justify-center hover:bg-gray-50 text-gray-600 transition font-bold space-x-1.5 text-sm">
                        <span>💬</span>
                        <span>Bình luận (<?php echo e($commentsCount); ?>)</span>
                    </button>
                </div>
                ...
                <div class="flex items-center space-x-3 border border-gray-300 rounded-full w-max px-4 py-2 bg-gray-50">
                    ...
                    <span class="font-bold text-gray-700">Được xem <?php echo e(number_format($recipe->views_count)); ?> lần</span>
                </div>

                <div class="flex items-center space-x-3 py-2">
                    <?php if($recipe->user->avatar): ?>
                        <img src="<?php echo e(str_contains($recipe->user->avatar, 'http') ? $recipe->user->avatar : Storage::url($recipe->user->avatar)); ?>" class="w-12 h-12 rounded-full border border-gray-200 object-cover">
                    <?php else: ?>
                        <div class="w-12 h-12 rounded-full border border-gray-200 bg-orange-100 text-orange-600 font-bold flex items-center justify-center text-lg">
                            <?php echo e(substr($recipe->user->name, 0, 1)); ?>

                        </div>
                    <?php endif; ?>
                    <div class="space-y-1">
                        <div class="font-bold text-gray-800"><?php echo e($recipe->user->name); ?></div>
                        <div class="flex items-center space-x-1 text-sm text-gray-500">
                            <span>Đăng ngày <?php echo e($recipe->created_at->format('d/m/Y')); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-6 pt-4 text-sm font-bold text-gray-600">
                    <div class="flex items-center gap-2">
                        ...
                        <span><?php echo e($recipe->time_to_cook); ?> phút</span>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <span>Độ khó: <?php echo e(ucfirst($recipe->difficulty)); ?></span>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-3 pt-6 border-t border-gray-100" x-data="{ copied: false }">
                    ...
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="border border-gray-300 text-gray-600 hover:bg-gray-50 px-4 py-2 rounded text-sm font-bold flex items-center space-x-2 transition">
                            <span>🔖</span>
                            <span>Lưu Món</span>
                        </a>
                    <?php endif; ?>
            ...
            <div class="w-full lg:w-[350px] shrink-0 border border-gray-300 p-5 flex flex-col rounded bg-white relative max-h-[600px]">
                ...
                <div class="flex-1 overflow-y-auto pr-4 space-y-0 custom-scrollbar">
                    <?php $__empty_1 = true; $__currentLoopData = $recipe->ingredients; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ingredient): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="flex items-center justify-between border-b border-dashed border-gray-200 py-3 last:border-0">
                            <span class="font-medium text-gray-800"><?php echo e($ingredient->name); ?></span>
                            <span class="text-sm font-bold text-gray-500 bg-gray-50 px-2 py-1 rounded"><?php echo e($ingredient->pivot->quantity); ?> <?php echo e($ingredient->pivot->unit); ?></span>
                        </div>
        ...
        <div class="border-t border-gray-200 pt-8 relative">
            ...
            <div x-data="{ completedSteps: [] }" class="relative space-y-8 pl-2">
                ...
                <?php $__empty_1 = true; $__currentLoopData = $recipe->steps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $step): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php
                        // Render **bold** text
                        $content = e($step->content);
                        $content = preg_replace('/\*\*(.+?)\*\*/', '<strong class="text-gray-900 font-extrabold">$1</strong>', $content);
                    ?>
                    <div class="relative flex gap-6 group z-10 <?php echo e((!$isAuthorized && $step->order > $lockAfterStep) ? 'blur-md pointer-events-none select-none' : ''); ?>">
                        <!-- Step Indicator -->
                        <div class="flex flex-col items-center">
                            <button 
                                @click="completedSteps.includes(<?php echo e($step->order); ?>) ? completedSteps = completedSteps.filter(s => s !== <?php echo e($step->order); ?>) : completedSteps.push(<?php echo e($step->order); ?>)"
                                ...
                                :class="completedSteps.includes(<?php echo e($step->order); ?>) ? 'bg-green-500 border-green-500 text-white scale-110 shadow-lg shadow-green-500/20' : 'bg-white border-orange-500 text-orange-500 hover:bg-orange-50 group-hover:scale-105'"
                            >
                                <span x-show="!completedSteps.includes(<?php echo e($step->order); ?>)"><?php echo e($step->order); ?></span>
                                <span x-show="completedSteps.includes(<?php echo e($step->order); ?>)" style="display: none;">✓</span>
                            </button>
                        </div>
                        ...
                        <div 
                            ...
                            :class="completedSteps.includes(<?php echo e($step->order); ?>) ? 'border-green-200 bg-green-50/10 opacity-60' : 'border-gray-150 hover:border-orange-200 hover:bg-orange-50/5 shadow-sm hover:shadow-md'"
                        >
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    <span class="text-xs font-black uppercase tracking-wider transition" :class="completedSteps.includes(<?php echo e($step->order); ?>) ? 'text-green-600' : 'text-orange-500'">
                                        Bước <?php echo e($step->order); ?>

                                    </span>
                                    <span x-show="completedSteps.includes(<?php echo e($step->order); ?>)" class="bg-green-100 text-green-700 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wide" style="display: none;">
                                        Đã Xong ✓
                                    </span>
                                </div>
                                <p class="text-gray-700 leading-relaxed text-sm sm:text-base transition" :class="completedSteps.includes(<?php echo e($step->order); ?>) ? 'line-through text-gray-400' : ''">
                                    <?php echo $content; ?>

                                </p>
                                <?php if($step->image): ?>
                                    <div class="rounded-2xl overflow-hidden border border-gray-100 mt-4 shadow-sm max-h-[300px] flex items-center justify-center">
                                        <img src="<?php echo e($step->image); ?>" class="w-full h-full object-cover aspect-video hover:scale-[1.02] transition-transform duration-500" alt="Minh họa Bước <?php echo e($step->order); ?>">
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <?php if(!$isAuthorized && $step->order == $lockAfterStep): ?>
                        <div class="relative z-30 max-w-xl mx-auto my-8 p-8 bg-white/95 border border-amber-200 shadow-[0_20px_50px_rgba(245,158,11,0.12)] rounded-3xl text-center space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <p class="text-gray-600 text-sm leading-relaxed font-medium">
                                    <?php if($isAuthorMasterChef): ?>
                                        Bạn có thể thực hiện món ăn cơ bản qua <?php echo e($lockAfterStep); ?> bước trên. Để xem trọn vẹn bí quyết chế biến nâng tầm Master Chef từ tác giả <strong class="text-gray-900 font-bold"><?php echo e($recipe->user->name); ?></strong>, vui lòng mua trực tiếp công thức này (Không hỗ trợ xem quảng cáo).
                                    <?php else: ?>
                                        Bạn có thể thực hiện món ăn cơ bản qua <?php echo e($lockAfterStep); ?> bước trên. Để xem trọn vẹn bí quyết chế biến nâng tầm Master Chef từ tác giả <strong class="text-gray-900 font-bold"><?php echo e($recipe->user->name); ?></strong>, hãy mua công thức hoặc xem hết quảng cáo 15s.
                                    <?php endif; ?>
                                </p>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-3 justify-center pt-2">
                                <?php if(auth()->guard()->guest()): ?>
                                    <a href="<?php echo e(route('login')); ?>" class="w-full sm:w-auto px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl transition shadow-lg shadow-orange-500/20 text-sm flex items-center justify-center">
                                        Đăng nhập để mua (<?php echo e(number_format($recipe->price, 0, ',', '.')); ?> đ)
                                    </a>
                                    <?php if(!$isAuthorMasterChef): ?>
                                        <a href="<?php echo e(route('login')); ?>" class="w-full sm:w-auto px-6 py-3 bg-gray-805 hover:bg-gray-900 text-white font-bold rounded-xl transition text-sm flex items-center justify-center gap-1.5">
                                            <span>Đăng nhập để xem Ad 📺</span>
                                        </a>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <form action="<?php echo e(route('recipe.purchase', $recipe->id)); ?>" method="POST" class="w-full sm:w-auto m-0">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" class="w-full px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl transition shadow-lg shadow-orange-500/20 text-sm">
                                            Mua công thức (<?php echo e(number_format($recipe->price, 0, ',', '.')); ?> đ)
                                        </button>
                                    </form>
                                    
                                    <?php if(!$isAuthorMasterChef): ?>
                                        <?php if($adUnlocksCount >= 3): ?>
        ...
        <div id="comments-section" class="border-t border-gray-200 pt-8 space-y-6">
            ...
            <?php if(auth()->guard()->check()): ?>
                <form action="<?php echo e(route('comment.store', $recipe->id)); ?>" method="POST" class="flex items-center space-x-4">
                    <?php echo csrf_field(); ?>
                    <?php if(auth()->user()->avatar): ?>
                        <img src="<?php echo e(str_contains(auth()->user()->avatar, 'http') ? auth()->user()->avatar : Storage::url(auth()->user()->avatar)); ?>" class="w-12 h-12 rounded-full border border-gray-300 object-cover shrink-0">
                    <?php else: ?>
            ...
            <?php else: ?>
                <div class="bg-gray-50 border border-gray-200 rounded-2xl p-6 text-center space-y-3">
                    ...
                    <a href="<?php echo e(route('login')); ?>" class="inline-block bg-orange-500 hover:bg-orange-600 text-white font-bold px-6 py-2.5 rounded-xl transition shadow-md shadow-orange-500/20 text-sm">
                        Đăng nhập ngay
                    </a>
                </div>
            <?php endif; ?>
            ...
            <div class="space-y-4 pt-6">
                <?php $__empty_1 = true; $__currentLoopData = $recipe->comments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php
                    ...
                    ?>
                    <div x-data="{
                        likes: <?php echo e($comment->likes_count ?? 0); ?>,
                        dislikes: <?php echo e($comment->dislikes_count ?? 0); ?>,
                        userReaction: '<?php echo e($hasLiked ? 'like' : ($hasDisliked ? 'dislike' : '')); ?>',
                        isEditing: false,
                        commentContent: '<?php echo e(str_replace(["'", "\r", "\n"], ["\\'", " ", " "], $comment->content)); ?>',
                        editContent: '<?php echo e(str_replace(["'", "\r", "\n"], ["\\'", " ", " "], $comment->content)); ?>',
                        isDeleted: false,
                        async react(type) {
                            try {
                                const response = await fetch('<?php echo e(route('comment.react', $comment->id)); ?>', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                        ...
                        async updateComment() {
                            ...
                            try {
                                const response = await fetch('<?php echo e(route('comment.update', $comment->id)); ?>', {
                                    method: 'PUT',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                        ...
                        async deleteComment() {
                            ...
                            try {
                                const response = await fetch('<?php echo e(route('comment.destroy', $comment->id)); ?>', {
                                    method: 'DELETE',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                    ...
                    class="flex gap-4 p-4 bg-gray-50/50 rounded-2xl border border-gray-100 hover:border-orange-100 transition-all">
                        <?php if($comment->user->avatar): ?>
                            <img src="<?php echo e(str_contains($comment->user->avatar, 'http') ? $comment->user->avatar : Storage::url($comment->user->avatar)); ?>" class="w-10 h-10 rounded-full border border-gray-200 object-cover shrink-0">
                        <?php else: ?>
                        ...
                        <div class="flex-1 space-y-1">
                            <div class="flex items-center justify-between">
                                <span class="font-bold text-gray-800 text-sm"><?php echo e($comment->user->name); ?></span>
                            ...
                            <div class="flex items-center space-x-6 pt-2 text-xs font-bold text-gray-500">
                                ...
                                <?php else: ?>
                                    <a href="<?php echo e(route('login')); ?>" class="flex items-center space-x-1 hover:text-orange-500 transition">
                                        <span>👍 Thích (<?php echo e($comment->likes_count ?? 0); ?>)</span>
                                    </a>
                                <?php endif; ?>
                                ...
                                <?php else: ?>
                                    <a href="<?php echo e(route('login')); ?>" class="flex items-center space-x-1 hover:text-red-500 transition">
                                        <span>👎 Không thích (<?php echo e($comment->dislikes_count ?? 0); ?>)</span>
                                    </a>
                                <?php endif; ?>
                                ...
                                <?php if(auth()->guard()->check()): ?>
                                    ...
                                    <?php else: ?>
                                        <button @click="$dispatch('open-report-modal', { commentId: <?php echo e($comment->id); ?> })" class="flex items-center space-x-1 hover:text-red-500 transition">
                                            <span>🚩 Tố cáo</span>
                                        </button>
                                    <?php endif; ?>
                                <?php else: ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\--main\resources\views/recipe/detail.blade.php ENDPATH**/ ?>