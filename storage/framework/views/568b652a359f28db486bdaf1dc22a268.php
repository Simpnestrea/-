<?php $__env->startSection('title', 'Tổng quan Quản trị - Foodball'); ?>
<?php $__env->startSection('page_title', 'Tổng quan Quản trị'); ?>
...
<div class="space-y-6" x-data="dashboardGreeting()">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="space-y-2">
                ...
                <h2 class="text-2xl md:text-3xl font-black tracking-tight">
                    <span x-text="greeting"></span>, <span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-300"><?php echo e(auth()->user()->name); ?></span>!
                </h2>
                <p class="text-slate-300 text-xs md:text-sm max-w-xl font-medium">
                    Chào mừng trở lại bảng điều khiển Foodball. Hệ thống của bạn đang quản lý <span class="text-white font-bold"><?php echo e($stats['total_users']); ?></span> thành viên và đã có <span class="text-white font-bold"><?php echo e(number_format($stats['total_revenue'], 0, ',', '.')); ?> đ</span> doanh thu từ bán công thức.
                </p>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition duration-200">
            <div class="space-y-2">
                ...
                <div class="flex items-baseline space-x-2">
                    <span class="text-3xl font-black text-slate-900 font-mono"><?php echo e($stats['total_users']); ?></span>
                </div>
                <p class="text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-100 rounded-full px-2 py-0.5 inline-block">
                    👑 <?php echo e($stats['premium_users']); ?> Premium
                </p>
            </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition duration-200">
            <div class="space-y-2">
                ...
                <div class="flex items-baseline space-x-2">
                    <span class="text-3xl font-black text-slate-900 font-mono"><?php echo e($stats['total_recipes']); ?></span>
                </div>
                <p class="text-[10px] text-slate-400 font-medium">Trung bình <?php echo e($stats['total_users'] > 0 ? round($stats['total_recipes'] / $stats['total_users'], 1) : 0); ?> món/thành viên</p>
            </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition duration-200">
            <div class="space-y-2">
                ...
                <div class="flex items-baseline space-x-1">
                    <span class="text-2xl font-black text-slate-900 font-mono"><?php echo e(number_format($stats['total_revenue'], 0, ',', '.')); ?></span>
                    <span class="text-[10px] font-bold text-slate-400">đ</span>
                </div>
                <p class="text-[10px] text-slate-400 font-medium">Tổng tiền ví hệ thống: <?php echo e(number_format($stats['total_wallet_balance'], 0, ',', '.')); ?> đ</p>
            </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition duration-200">
            <div class="space-y-2">
                ...
                <div class="flex items-baseline space-x-2">
                    <span class="text-3xl font-black text-slate-900 font-mono"><?php echo e($stats['total_reports']); ?></span>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden lg:col-span-2 flex flex-col justify-between">
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                ...
                <a href="<?php echo e(route('admin.recipes')); ?>" class="text-xs font-bold text-orange-500 hover:text-orange-600 transition flex items-center gap-0.5">
            ...
            <div class="divide-y divide-slate-50 flex-1">
                <?php $__empty_1 = true; $__currentLoopData = $topRecipes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $recipe): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="px-6 py-4 flex items-center space-x-4 hover:bg-slate-50/50 transition">
                        <img src="<?php echo e($recipe->image ?? 'https://images.unsplash.com/photo-1495521821757-a1efb6729352?w=120'); ?>" class="w-12 h-12 rounded-xl object-cover bg-slate-100 border border-slate-200/60 shrink-0 shadow-sm">
                        <div class="flex-1 min-w-0">
                            <h4 class="font-bold text-sm text-slate-800 truncate"><?php echo e($recipe->title); ?></h4>
                            <p class="text-xs text-slate-500 flex items-center gap-1.5 mt-0.5">
                                <span>Tác giả: <strong class="text-slate-700 font-semibold"><?php echo e($recipe->user->name ?? 'Ẩn danh'); ?></strong></span>
                                <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                <span>Danh mục: <strong class="text-slate-700 font-semibold"><?php echo e($recipe->category->name ?? 'Không rõ'); ?></strong></span>
                            </p>
                        </div>
                        <div class="text-right shrink-0">
                            <span class="text-lg font-black text-slate-900 font-mono"><?php echo e(number_format($recipe->views_count)); ?></span>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden" x-data="dismissibleReports()">
        <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('admin.reports')); ?>" class="text-xs font-bold text-orange-500 hover:text-orange-600 transition flex items-center gap-0.5">
                Xem tất cả <span>→</span>
            </a>
        </div>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $recentReports; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $report): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/30 transition-all duration-200" id="report-row-<?php echo e($report->id); ?>">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-bold text-slate-800"><?php echo e($report->user->name ?? 'Thành viên ẩn danh'); ?></div>
                                <div class="text-[10px] text-slate-400"><?php echo e($report->created_at->diffForHumans()); ?></div>
                            </td>
                            <td class="px-6 py-4">
                                <?php if($report->comment): ?>
                                    <div class="text-slate-700 max-w-xs truncate font-medium" title="<?php echo e($report->comment->content); ?>">"<?php echo e($report->comment->content); ?>"</div>
                                    <div class="text-[10px] text-slate-400">Tác giả: <?php echo e($report->comment->user->name ?? 'Không rõ'); ?></div>
                                <?php else: ?>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php if($report->comment && $report->comment->recipe): ?>
                                    <a href="<?php echo e(route('recipe.detail', $report->comment->recipe->slug)); ?>" target="_blank" class="text-orange-500 font-semibold hover:underline block max-w-[150px] truncate">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    <button @click="dismiss(<?php echo e($report->id); ?>)" class="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-3 py-1.5 rounded-lg text-xs transition">
                                        Bỏ qua
                                    </button>
                                    <?php if($report->comment): ?>
                                        <button @click="deleteComment(<?php echo e($report->id); ?>)" class="bg-red-500 hover:bg-red-600 text-white font-bold px-3 py-1.5 rounded-lg text-xs transition">
                                            Xóa bình luận
                                        </button>
                                    <?php endif; ?>
                                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        new Chart(ctxUser, {
            type: 'line',
            data: {
                labels: <?php echo json_encode($chartUserGrowth['labels'], 15, 512) ?>,
                datasets: [{
                    label: 'Thành viên mới',
                    data: <?php echo json_encode($chartUserGrowth['data'], 15, 512) ?>,
                    borderColor: '#f97316',
                    borderWidth: 3,
        ...
        new Chart(ctxRatio, {
            ...
            data: {
                ...
                datasets: [{
                    data: [<?php echo json_encode($userRatio['premium'], 15, 512) ?>, <?php echo json_encode($userRatio['free'], 15, 512) ?>],
        ...
        new Chart(ctxRoles, {
            ...
            data: {
                ...
                datasets: [{
                    data: [
                        <?php echo json_encode($roleStats['beginner'] ?? 0, 15, 512) ?>,
                        <?php echo json_encode($roleStats['homecook'] ?? 0, 15, 512) ?>,
                        <?php echo json_encode($roleStats['prochef'] ?? 0, 15, 512) ?>,
                        <?php echo json_encode($roleStats['masterchef'] ?? 0, 15, 512) ?>
                    ],
        ...
        const categories = <?php echo json_encode($categoriesData, 15, 512) ?>;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\--main\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>