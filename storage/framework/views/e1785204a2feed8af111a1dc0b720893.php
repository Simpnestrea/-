<?php $__env->startSection('title', 'Quản lý Thành viên - Foodball'); ?>
<?php $__env->startSection('page_title', 'Quản lý Thành viên'); ?>
...
<div class="space-y-6" x-data="usersManager()">
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <form action="<?php echo e(route('admin.users')); ?>" method="GET" class="flex-1 flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                <input type="text" name="query" value="<?php echo e($query); ?>" placeholder="Tìm theo tên, username, email..." 
                ...
                <?php if($query): ?>
                    <a href="<?php echo e(route('admin.users')); ?>" class="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400 hover:text-slate-600">Xóa tìm kiếm</a>
                <?php endif; ?>
            </div>
            ...
            <select name="admin_role" onchange="this.form.submit()" class="py-2.5 pl-4 pr-10 rounded-xl border border-slate-200 outline-none text-sm transition-all focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10">
                ...
                <option value="admin" <?php echo e(($adminRole ?? '') === 'admin' ? 'selected' : ''); ?>>Chỉ Admin</option>
                <option value="user" <?php echo e(($adminRole ?? '') === 'user' ? 'selected' : ''); ?>>Chỉ Người dùng</option>
            </select>
            ...
            <select name="role" onchange="this.form.submit()" class="py-2.5 pl-4 pr-10 rounded-xl border border-slate-200 outline-none text-sm transition-all focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10">
                ...
                <option value="beginner" <?php echo e(($role ?? '') === 'beginner' ? 'selected' : ''); ?>>🔰 Người mới (Beginner)</option>
                <option value="homecook" <?php echo e(($role ?? '') === 'homecook' ? 'selected' : ''); ?>>🍳 Đầu bếp tại gia (Home Cook)</option>
                <option value="prochef" <?php echo e(($role ?? '') === 'prochef' ? 'selected' : ''); ?>>👨‍🍳 Chuyên nghiệp (Pro Chef)</option>
                <option value="masterchef" <?php echo e(($role ?? '') === 'masterchef' ? 'selected' : ''); ?>>👑 Siêu đầu bếp (Master Chef)</option>
            </select>
            ...
            <select name="status" onchange="this.form.submit()" class="py-2.5 pl-4 pr-10 rounded-xl border border-slate-200 outline-none text-sm transition-all focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10">
                ...
                <option value="premium" <?php echo e(($status ?? '') === 'premium' ? 'selected' : ''); ?>>Gói Premium 👑</option>
                <option value="free" <?php echo e(($status ?? '') === 'free' ? 'selected' : ''); ?>>Gói Thường</option>
            </select>
        </form>
        ...
        <div class="text-xs text-slate-400 font-bold uppercase tracking-wider shrink-0 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg">
            Hiển thị <?php echo e($users->firstItem() ?? 0); ?> - <?php echo e($users->lastItem() ?? 0); ?> / <?php echo e($users->total()); ?> thành viên
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    <?php $__empty_1 = true; $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/30 transition-all" id="user-row-<?php echo e($user->id); ?>">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-3">
                                    <div class="w-10 h-10 rounded-xl overflow-hidden bg-slate-100 border border-slate-200/80 shrink-0">
                                        <?php if($user->avatar): ?>
                                            <img src="<?php echo e(Storage::url($user->avatar)); ?>" alt="Avatar" class="w-full h-full object-cover">
                                        <?php else: ?>
                                            <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($user->name)); ?>&color=f97316&background=fff7ed" alt="Avatar" class="w-full h-full object-cover">
                                        <?php endif; ?>
                                    </div>
                                    <div>
                                        <div class="font-bold text-slate-800 hover:text-orange-500 cursor-pointer" @click="showDetail(<?php echo e($user->id); ?>)"><?php echo e($user->name); ?></div>
                                        <div class="text-xs text-slate-400">&#64;<?php echo e($user->username ?? 'social_user'); ?></div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <!-- Email & Joined Date -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-slate-700 font-medium"><?php echo e($user->email); ?></div>
                                <div class="text-[10px] text-slate-400">Tham gia: <?php echo e($user->created_at ? $user->created_at->format('d/m/Y') : 'Không rõ'); ?></div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="space-y-1.5">
                                    ...
                                    <div class="relative w-36">
                                        <select @change="changeRole(<?php echo e($user->id); ?>, $event)" 
                                            ...
                                            <option value="beginner" <?php echo e($user->role === 'beginner' ? 'selected' : ''); ?>>🔰 Beginner</option>
                                            <option value="homecook" <?php echo e($user->role === 'homecook' ? 'selected' : ''); ?>>🍳 Home Cook</option>
                                            <option value="prochef" <?php echo e($user->role === 'prochef' ? 'selected' : ''); ?>>👨‍🍳 Pro Chef</option>
                                            <option value="masterchef" <?php echo e($user->role === 'masterchef' ? 'selected' : ''); ?>>👑 Master Chef</option>
                                        </select>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center font-bold text-slate-700 font-mono">
                                <?php echo e($user->recipes_count); ?>

                            </td>

                            <!-- Comments count -->
                            <td class="px-6 py-4 whitespace-nowrap text-center font-bold text-slate-700 font-mono">
                                <?php echo e($user->comments_count); ?>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <!-- View detail button -->
                                    <button @click="showDetail(<?php echo e($user->id); ?>)" class="font-bold text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-700 transition">
                                        Xem chi tiết
                                    </button>

                                    <!-- Toggle Premium Action -->
                                    <form action="<?php echo e(route('admin.users.toggle-premium', $user)); ?>" method="POST" class="m-0 inline" @submit.prevent="togglePremium(<?php echo e($user->id); ?>, $event)">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" class="premium-btn font-bold text-xs px-2.5 py-1.5 rounded-lg border transition-all
                                            <?php echo e($user->is_premium 
                                                ? 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100' 
                                                : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50 hover:text-slate-800'); ?>">
                                            <?php echo e($user->is_premium ? 'Hủy Premium' : 'Gán Premium'); ?>
                                    ...
                                    <?php if(auth()->id() !== $user->id): ?>
                                        <form action="<?php echo e(route('admin.users.toggle-admin', $user)); ?>" method="POST" class="m-0 inline" @submit.prevent="toggleAdmin(<?php echo e($user->id); ?>, $event, '<?php echo e(addslashes($user->name)); ?>')">
                                            <?php echo csrf_field(); ?>
                                            <button type="submit" class="admin-btn font-bold text-xs px-2.5 py-1.5 rounded-lg border transition-all
                                                <?php echo e($user->is_admin 
                                                    ...
                                                    : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50 hover:text-slate-800'); ?>">
                                                <?php echo e($user->is_admin ? 'Hạ quyền Admin' : 'Thăng Admin'); ?>
                                    ...
                                    <!-- Delete Action -->
                                    <?php if(auth()->id() !== $user->id): ?>
                                        <form action="<?php echo e(route('admin.users.delete', $user)); ?>" method="POST" class="m-0 inline" @submit.prevent="deleteUser(<?php echo e($user->id); ?>, $event)">
                                            <?php echo csrf_field(); ?>
        ...
        <?php if($users->hasPages()): ?>
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                <?php echo e($users->appends(['query' => $query, 'admin_role' => $adminRole ?? '', 'role' => $role ?? '', 'status' => $status ?? ''])->links()); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\--main\resources\views/admin/users.blade.php ENDPATH**/ ?>